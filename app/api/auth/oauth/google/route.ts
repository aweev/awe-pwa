...
import { exchangeGoogleCode, getGoogleUserInfo, oauthService } from '@/lib/auth/providers/oauth.service';
...
export async function GET(request: NextRequest) {
  ...
  const state = searchParams.get('state'); // Contains the returnTo URL
  ...
  if (error) {
    await auditLog('oauth_error', undefined, { provider: 'google', error, ip });
    return NextResponse.redirect(new URL(`/login?error=oauth_access_denied`, request.url));
  }

  if (!code) {
    await auditLog('oauth_missing_code', undefined, { provider: 'google', ip });
  ...
  try {
    ...
    const { refreshToken } = await oauthService.handleLogin('google', normalizedData, ip, userAgent ?? undefined);
    ...
    await auditLog('oauth_exchange_failed', undefined, { provider: 'google', error: String(err), ip });
